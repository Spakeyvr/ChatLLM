...
struct On_Device_LLM_ChatTests {
    ...
    @Test func rewindingForEditRestoresTextAndDeletesTheSelectedTurnAndEverythingAfterIt() async throws {
        let schema = Schema([Conversation.self, On_Device_LLM_Chat.Message.self, MessageAttachment.self])
        let container = try ModelContainer(
            for: schema,
            configurations: [ModelConfiguration(isStoredInMemoryOnly: true)]
        ...
        let restoredText = viewModel.rewindForEditingUserMessage(messageID: messages[2].id)
        ...
        #expect(Set(persistedMessages.map(\.id)) == Set(messages.prefix(2).map(\.id)))

        conversation.title = "Generated title"
        conversation.hasAutoGeneratedTitle = true
        let firstMessageText = viewModel.rewindForEditingUserMessage(messageID: messages[0].id)
