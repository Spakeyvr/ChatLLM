...
device_info(int device_index) {
  auto init_device_info = []()
    ...
    const char* configured_arch = std::getenv("MLX_METAL_GPU_ARCH");
    auto arch = configured_arch != nullptr && configured_arch[0] != '\0'
        ? std::string(configured_arch)
        : std::string(raw_device->architecture()->name()->utf8String());
