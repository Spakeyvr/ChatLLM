...
void Gather::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  if (src.flags().row_contiguous && nidx == 1 && axes_[0] == 0 &&
    ...
    if (kernel_name == "gather_frontfloat16_int32_int_2") {
      kernel = d.get_kernel(kernel_name);
    } else {
      kernel = d.get_kernel(kernel_name, d.get_library(lib_name, build_library));
    }
#else
    kernel = d.get_kernel(kernel_name, d.get_library(lib_name, build_library));
#endif
...
void Scatter::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  MTL::Size group_dims =
      get_block_dims(upd_size, grid_y, 1, thread_group_power);
  compute_encoder.dispatch_threads(grid_dims, group_dims);
}
