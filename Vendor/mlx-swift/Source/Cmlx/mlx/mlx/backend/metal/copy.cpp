...
void copy_gpu_inplace(
  ...
  if (ctype == CopyType::General || ctype == CopyType::GeneralGeneral) {
    ...
    // Pipeline limits are powers of two. The iOS Simulator advertises a
    // smaller maximum than physical Apple GPUs, so size the 3D launch to the
    // actual pipeline rather than assuming 1024 threads.
    int thread_group_power = 0;
    for (auto size = thread_group_size; size > 1; size >>= 1) {
      thread_group_power++;
    }
    auto group_dims = get_block_dims(dim0, dim1, rest, thread_group_power);
