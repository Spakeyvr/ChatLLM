...
public struct GenerateParameters: Sendable {
    ...
    /// Explicit cache compression mode used by generation.
    ///
    /// When nil, legacy `kvBits` / `kvGroupSize` / `quantizedKVStart` settings
    /// are still honored for backward compatibility.
    public var cacheCompression: KVCacheCompressionMode?
    ...
    public var resolvedCacheCompression: KVCacheCompressionMode? {
        if let cacheCompression {
            return cacheCompression
        }
        if let kvBits {
            return .quantized(bits: kvBits, groupSize: kvGroupSize, startStep: quantizedKVStart)
        }
        return nil
    }
...
public struct TokenIterator: Sequence, IteratorProtocol {
    ...
    public init(
        ...
        self.cacheCompression = parameters.resolvedCacheCompression
    ...
    public init(
        ...
        // No cache compression for this direct initialization.
        self.cacheCompression = nil
